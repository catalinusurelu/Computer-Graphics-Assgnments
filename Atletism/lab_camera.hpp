...
namespace lab{
	...
	public:
		...
		void translateForward(float distance){
			//translatie
			//TODO
			 //New_Observator = Observator + normalizare(Forward)*distanta
		...
		void rotateFPSoX(float angle){
			...
			forward = forward * cos(glm::radians(angle)) + up * sin(glm::radians(angle));
		...
		void rotateFPSoY(float angle){
			...
			//TODO
			//e bun
			forward = forward * cos(glm::radians(angle)) + right * sin(glm::radians(angle));
			right = glm::cross(forward, up);
			//update geometrie
			updateGeometry();
		}
		void rotateFPSoZ(float angle){
			...
			up = up * cos(glm::radians(angle)) + right * sin(glm::radians(angle));
			right = glm::cross(forward, up);
		...
		void rotateTPSoX(float angle, float distance){
			...
			distance = glm::distance(position, glm::vec3(0,0,0));
			glm::vec3 old_forward;
			old_forward = forward;

			rotateFPSoX(angle);
			position = position + distance * old_forward - distance * forward; 
