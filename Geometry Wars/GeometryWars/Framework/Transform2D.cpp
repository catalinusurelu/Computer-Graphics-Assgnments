...
void Transform2D::applyTransform_o(Movable *movable)
{
    if(dynamic_cast<Ship*>(movable) != NULL)
    {
        Ship* ship = dynamic_cast<Ship*>(movable);
         Components::iterator it;
         for(it = ship->parts.begin(); it != ship->parts.end(); it++)
            applyTransform_o(*it);
    }
    else //if(dynamic_cast<Torpedo*>(movable) != NULL)
    {
         Torpedo* torpedo = dynamic_cast<Torpedo*>(movable);
         Components::iterator it;
         for(it = torpedo->parts.begin(); it != torpedo->parts.end(); it++)
            applyTransform_o(*it);
    }
}