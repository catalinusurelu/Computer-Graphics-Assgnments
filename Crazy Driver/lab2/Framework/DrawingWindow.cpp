...
DrawingWindow::DrawingWindow(int argc, char** argv,int _width, int _height, int startX, int startY, string name)
{
	...
	glutKeyboardFunc(onKeyDown); //functia apelata la apasarea unei taste
	glutSpecialFunc(onKeySpecialDown); //functia apelata la apasarea unei taste speciale
    glutSpecialUpFunc(onKeySpecialUp);
	glutMouseFunc(mouseFunction); //functia apelata la click de mouse

	glClearColor(FUNDAL.r,FUNDAL.g,FUNDAL.b,1);  //afiseaza fondul ferestrei aplicatiei in alb
